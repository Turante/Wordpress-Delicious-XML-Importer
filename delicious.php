...
Author: Guillermo Moreno, Stephanie Leary, Aaron Parecki
 ...
 *
 * 0.2.2 Updated to add bookmars as a custom post type, requires the "bookmarks" post type first.
 */
...
class Delicious_Import extends WP_Importer {
	...
	function greet() {
		...
		<?php printf(__('<a href="%s">Visit this link to get an XML file of your bookmarks.</a> You will be asked to log in to your Delicious account. 
		If you created your account after Yahoo acquired Delicious, you should use the <a href="%s">Delicious XML Export service</a> instead.', 'delicious-xml-importer'), 'https://api.del.icio.us/v1/posts/all?meta=yes', 'http://deliciousxml.com/'); ?></p>
		...
		<p><label for="bookmarksposts"><?php _e("Import your Delicious Bookmarks as: ", 'delicious-xml-importer'); ?> </label>
			<select name="bookmarksposts">
				<option value="posts"><?php _e("Posts", 'delicious-xml-importer'); ?></option>
				<option value="bookmark"><?php _e("Bookmarks (custom post type)", 'delicious-xml-importer'); ?></option>
				<option value="links"><?php _e("Links", 'delicious-xml-importer'); ?></option>
			</select>
		</p>
		
		<p><label for="categoriestags"><?php _e("If posts, import your Delicious Tags as: ", 'delicious-xml-importer'); ?></label>
	...
	function get_posts() {
		...
		$link_url = (string)$post['href'];
			...
			$this->posts[$index] = compact('post_author', 'post_date', 'post_date_gmt', 'post_content', 'post_title', 'post_status', 'categories', 'link_url');
			$index++;
		}
	}
	...
	function import_posts() {
		...
		foreach ($this->posts as $post) {
			echo "<li>".__('Importing post...', 'delicious-xml-importer');
			...
			if ($post_id = post_exists($post_title, $post_content, $post_date)) {
				...
				if($_POST['bookmarksposts'] == 'bookmark')
				...
				if($_POST['bookmarksposts'] == 'bookmark')
					add_post_meta($post_id, 'link_url', $post['link_url']);	
	...
	function import_links() {
		...
		foreach ($this->posts as $post) {
			echo "<li>".__('Importing link ', 'delicious-xml-importer');
	...
	function import() {
		...
		echo '<h3>';
		printf(__('All done. <a href="%s">Have fun!</a>', 'delicious-xml-importer'), 'link-manager.php');
		echo '</h3>';
	}
